...
const { herolist } = require('./database/lib/herolist.js')
const { herodetails } = require('./database/lib/herodetail.js')
const { yta, ytv, buffer2Stream, ytsr, baseURI, stream2Buffer, noop } = require('./database/lib/ytdl')
const { wikiSearch } = require('./database/lib/wiki.js')
...
const { help, bahasa, donasi, limitend, limitcount, bottt } = require('./database/lib/help')
...
│❒ ${a}${prefix}cerpen${a}
│❒ ${a}${prefix}carigrup [name]${a}
│❒ ${a}${prefix}playstore [name]${a}
│❒ ${a}${prefix}google [name]${a}
│❒ ${a}${prefix}googleimage [name]${a}
│❒ ${a}${prefix}trendtwit${a}
│❒ ${a}${prefix}imguploader [image]${a}
│❒ ${a}${prefix}herolist${a}
│❒ ${a}${prefix}herodetail [Hero ml]${a}
...
case prefix+'tagall':
			    ...
			    //if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ *Cie kena tag* ✪══`+ teks +'╚═══〘 ${name} BOT 〙═══', members_id, true)
					break
case prefix+'add':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
